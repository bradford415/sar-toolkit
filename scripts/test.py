...
def pga(img, win = 'auto', win_params = [100, 0.5], shadow_pga = False):
    ...
    for iteration in range(max_iter):
        ...
        for sample in range(nsamples):
            shifted_image[:,sample] = np.roll(img_af[:,sample], int(npulses/2-index[sample]))
        ...
        if win == 'auto':
            ...
            window = np.arange(npulses/2-win_width/2,npulses/2+win_width/2).astype(np.int32)
        else:
        ...
        phi_dot = np.angle(np.sum(np.conj(windowed_signals[:-1, :]) * windowed_signals[1:, :], axis=1)) 
